...
export default function FCFSScheduler() {
  ...
  const addProcess = () => {
    ...
    let updatedProcesses = [...processes, newProcess]
      .sort((a, b) => a.arrivalTime - b.arrivalTime || a.originalId - b.originalId);

    updatedProcesses = calculateFCFS(updatedProcesses);
  ...
  const calculateFCFS = (processes: Process[]): Process[] => {
    let completionTime = 0;
    return processes.map((process, index) => {
      if (index === 0 || process.arrivalTime > completionTime) {
  ...
  const getExecutionOrder = () => {
    ...
    const sortedProcesses = [...processes].sort((a, b) => 
      a.arrivalTime - b.arrivalTime || a.originalId - b.originalId);
  ...
  return (
    ...
    >
      ...
      <div className='min-h-[100vh] p-5 m-auto max-w-7xl flex items-center justify-center'>
        <Card 
          title={
        ...
        >
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            <Text style={{ color: 'rgba(255, 255, 255, 0.65)' }}>
              First-Come, First-Served (FCFS) is a scheduling algorithm where the process that arrives first is executed first.
